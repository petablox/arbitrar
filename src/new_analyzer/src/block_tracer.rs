...
impl<'ctx> GenerateBlockTraceTrait<'ctx> for CompositeBlockTrace<'ctx> {
  fn block_traces(&self) -> Vec<BlockTrace<'ctx>> {
    let func_num_block_traces : Vec<usize> = self.iter().map(|func_blk_trace| func_blk_trace.block_traces.len()).collect();
    ...
    for indices in utils::cartesian(&func_num_block_traces) {
      println!("{:?}", indices);
      let block_trace = indices.iter().enumerate().map(|(i, j)| {
        FunctionBlockTrace {
...
impl<'a, 'ctx> BlockTracer<'a, 'ctx> {
  pub fn block_graph_of_function(&self, f: Function<'ctx>) -> BlockGraph<'ctx> {
    ...
    for block in f.iter_blocks() {
      let block_id = block_id_map.entry(block).or_insert_with(|| graph.add_node(block)).clone();
      let terminator = block.last_instruction().unwrap();
      let next_blocks = block.destination_blocks();
      for next_block in next_blocks {
        let next_block_id = block_id_map.entry(next_block).or_insert_with(|| graph.add_node(next_block)).clone();
  ...
  pub fn block_traces_of_function_call(
    ...
    if entry_block == instr.parent_block() {
      ...
      petgraph::algo::all_simple_paths(
        ...
        path.into_iter().map(|ni| block_graph.graph[ni]).collect()
      }).collect()
    }
  }
  ...
  pub fn block_traces_of_function_trace(&self, slice: &Slice<'ctx>, func_trace: FunctionTrace<'ctx>) -> Vec<BlockTrace<'ctx>> {
    ...
    for (call_instr, next_func) in func_trace.functions {
      let block_traces = self.block_traces_of_function_call(curr_func, call_instr);
      comp_trace.push(CompositeFunctionBlockTraces { function: curr_func, block_traces, call_instr });
      curr_func = next_func;
    }
  ...
  pub fn function_traces(&self, slice: &Slice<'ctx>) -> Vec<FunctionTrace<'ctx>> {
    if slice.entry == slice.caller {
      vec![
        FunctionTrace {
          begin: slice.entry,
          functions: vec![(slice.instr, slice.callee)]
        }
      ]
    } else {
      petgraph::algo::all_simple_paths(
      ...
      ).filter_map(|path: Vec<_>| {

        // Function path:
        // The first element must be the entry of the slice
        // The last element must be the callee of the slice
        // And the second last element must be the caller
        if self.call_graph.graph[path[path.len() - 2]] == slice.caller {
          ...
          for i in 0..path.len() - 1 {
            ...
            let function = self.call_graph.graph[path[i + 1]];
            if slice.functions.contains(&function) {
              functions.push((call_instr, function));
            } else {
              return None
            }
          }
          Some(FunctionTrace { begin, functions })
        } else {
          None
        }
      }).collect()
