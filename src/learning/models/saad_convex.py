...
class ConvexSSAD:
  ...
  def __init__(self, kernel, y, kappa=1.0, Cp=1.0, Cu=1.0, Cn=1.0, verbose=0):
    ...
    if self.labeled == 0:
      ...
      self.kappa = 0.0
    self.print('Convex semi-supervised anomaly detection with {0} samples ({1} labeled).'.format(
  ...
  def fit(self, check_psd_eigs=False):
    ...
    self.print('- sum_(i in sv) alpha_i cy_i = {0} ~ 1.0 (approx error)'.format(
        np.sum(self.alphas[self.svs] * self.cy[self.svs])))
    self.print('- sum_(i in labeled) alpha_i = {0} >= {1} = kappa'.format(np.sum(self.alphas[self.cl == 1]),
                                                                          self.kappa))
