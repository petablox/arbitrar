...
class ConvexSSAD:
    ...
    def __init__(self, kernel, y, kappa=1.0, Cp=1.0, Cu=1.0, Cn=1.0, verbose=0):
        ...
        # prohibit a solution
        if self.labeled == 0:
            self.print('There are no labeled examples hence, setting kappa=0.0')
            self.kappa = 0.0
        self.print('Convex semi-supervised anomaly detection with {0} samples ({1} labeled).'.format(self.samples, self.labeled))

    def set_train_kernel(self, kernel):
        dim1, dim2 = kernel.shape
        self.print([dim1, dim2])
    ...
    def fit(self, check_psd_eigs=False):
        ...
        self.print(self.kernel.shape, Y.shape)
        ...
        if check_psd_eigs:
            ...
            if eigs[0] < 0.0:
                self.print('Smallest eigenvalue is {0}'.format(eigs[0]))
        ...
        self.print('Validate solution:')
        self.print('- found {0} support vectors'.format(len(self.svs)))
        self.print('0 <= alpha_i : {0} of {1}'.format(np.sum(0. <= self.alphas), N))
        self.print('- sum_(i) alpha_i cy_i = {0} = 1.0'.format(np.sum(self.alphas*self.cy)))
        self.print('- sum_(i in sv) alpha_i cy_i = {0} ~ 1.0 (approx error)'.format(np.sum(self.alphas[self.svs]*self.cy[self.svs])))
        self.print('- sum_(i in labeled) alpha_i = {0} >= {1} = kappa'.format(np.sum(self.alphas[self.cl == 1]), self.kappa))
        self.print('- sum_(i in unlabeled) alpha_i = {0}'.format(np.sum(self.alphas[self.y == 0])))
        self.print('- sum_(i in positives) alpha_i = {0}'.format(np.sum(self.alphas[self.y == 1])))
        self.print('- sum_(i in negatives) alpha_i = {0}'.format(np.sum(self.alphas[self.y ==-1])))
        ...
        # case 2: only labeled examples available
            ...
            if ninds.size > 0 and pinds.size == 0:
                self.print('ERROR: Check pre-defined PRECISION.')
