...
module Stmt = struct
  ...
  let json_of_unop cache instr name =
    let opcode = ("opcode", `String name) in
    let result = `String (Utils.EnvCache.string_of_exp cache instr) in
    let op0 =
      `String (Utils.EnvCache.string_of_exp cache (Llvm.operand instr 0))
    in
    `Assoc [opcode; ("result", result); ("op0", op0)]

  let json_of_binop cache instr name =
    let opcode = ("opcode", `String name) in
    let result = `String (Utils.EnvCache.string_of_lhs cache instr) in
    let op0 =
      `String (Utils.EnvCache.string_of_exp cache (Llvm.operand instr 0))
    in
    let op1 =
      `String (Utils.EnvCache.string_of_exp cache (Llvm.operand instr 1))
    in
  ...
  let json_of_instr cache instr =
    ...
    | Br ->
        ...
        let cond =
          ...
          | Some (`Conditional _) ->
              `String
                (Utils.EnvCache.string_of_exp cache (Llvm.operand instr 0))
          | _ ->
              `Null
        in
    ...
    | Call -> (
        ...
        let result =
          ...
          | Llvm.TypeKind.Void ->
              `Null
          | _ ->
              `String (Utils.EnvCache.string_of_lhs cache instr)
        in
