...
class Database:
    ...
    def func_bc_slice_dugraphs_dir(self,
                                   func: str,
                                   bc_name: str,
                                   slice_id: int,
                                   create=False) -> str:
    ...
    def func_bc_slice_features_dir(self,
                                   func: str,
                                   bc_name: str,
                                   slice_id: int,
                                   create=False) -> str:
    ...
    def feature_dir(self, func: str, bc_name: str, slice_id: int,
                    trace_id: int) -> str:
    ...
    def num_slices(self, func_name=None, bc=None) -> int:
        if func_name != None and bc != None:
            ...
            for root, dirs, files in os.walk(
                    self.func_bc_slices_dir(func_name, bc)):
                for f in files:
                    count += 1
            return count
        elif func_name != None:
    ...
    def num_traces(self, func_name=None, bc=None):
        if func_name != None and bc != None:
            count = 0
            for root, dirs, files in os.walk(
                    self.func_bc_dugraphs_dir(func_name, bc)):
                for f in files:
                    count += 1
            return count
        elif func_name != None:
            count = 0
            for root, dirs, files in os.walk(
                    self.func_dugraphs_dir(func_name)):
                for f in files:
                    count += 1
            return count
        elif bc != None:
    ...
    def function_datapoints(self, func_name: str):
        ...
        for bc in os.listdir(func_slices_dir):
            ...
            for slice_name in os.listdir(bc_dir):
                ...
                trace_dir = self.func_bc_slice_dugraphs_dir(
                    func_name, bc, slice_id)
                for trace_name in os.listdir(trace_dir):
                    trace_id = int(os.path.splitext(trace_name)[0])
                    yield DataPoint(self,
                                    func_name,
                                    bc,
                                    slice_id,
                                    trace_id,
